...
/** Short-lived signed download link for an attachment, after a permission check. */
export async function GET(_req: NextRequest, ctx: RouteContext<"/api/files/[id]">) {
  ...
  const { data, error } = await db().storage.from("task-files").createSignedUrl(file.storage_path, 120, { download: file.name });
